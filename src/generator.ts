// generator.ts - Tiling and substitution logic for generating Spectre patterns
...
/**
 * Build the base Spectre tile system
 */
export function buildSpectreBase(curved: boolean): Record<string, Shape | CurvyShape | Meta> {
  ...
  const ret: Record<string, Shape | CurvyShape | Meta> = {};
...
export function buildHatTurtleBase(hat_dominant: boolean): Record<string, Shape | Meta> {
  ...
  const ret: Record<string, Shape | Meta> = {};
...
/**
 * Build the hexagon tile system
 */
export function buildHexBase(): Record<string, Shape> {
  const hr3 = 0.8660254037844386;
...
export function buildSupertiles(
  sys: Record<string, Shape | CurvyShape | Meta>
): Record<string, Meta> {
  const quad = (sys['Delta'] as Shape | CurvyShape).quad;
